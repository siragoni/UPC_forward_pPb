...
//  fModelForHighPtTail->SetParameter(2,debug==4 ? 6.1 : 1.);
   ...
   * -
   */
  TF1* fModelForHighPtTailPreliminary = new TF1("fModelForHighPtTailPreliminary","[0]*x*(1+[1]/[2]*x*x)^(-[2])",0.0,4.);
  ...
  fModelForHighPtTailPreliminary->SetNpx( 4.0/0.1 );
  fHighPtTailPreliminary = (TH1F*) fModelForHighPtTailPreliminary->GetHistogram()->Clone("fHighPtTailPreliminary");
  Double_t TotalPreliminaryIntegral  = 0.;
  Double_t HighPtPreliminaryIntegral = 0.;
  for (Int_t ibin=1; ibin<=fHighPtTailPreliminary->GetNbinsX(); ibin++) {
    fHighPtTailPreliminary->SetBinError(ibin,0);
    TotalPreliminaryIntegral += fHighPtTailPreliminary->GetBinContent(ibin);
    if ( ibin > 10 ) HighPtPreliminaryIntegral += fHighPtTailPreliminary->GetBinContent(ibin);
  }
  ...
  fDimuonPtDistributionDataCloneH->Fit(FitPtDistrPreliminary, "","", 1.25, 1.5);
  ...
  // if ( selectionFlag == 16) {
  //   FitPtDistr->SetParLimits(1, FitPtDistr->GetParameter(1)*0.7, FitPtDistr->GetParameter(1)*1.0);
  // } else {
  //   FitPtDistr->SetParLimits(1, FitPtDistr->GetParameter(1)*0.7, FitPtDistr->GetParameter(1)*1.0);
  // }

  // FitPtDistr->SetParameter(3, FitPtDistrPreliminary->GetParameter(0));
  // FitPtDistr->SetParLimits(3, FitPtDistrPreliminary->GetParameter(0)*0.8, FitPtDistrPreliminary->GetParameter(0)*1.2);
  FitPtDistr->SetParameter(3, 1);
  FitPtDistr->SetParLimits(3, 0., 1.);
  ...
  fDimuonPtDistributionDataH->Fit( FitPtDistr,"","", 0./*1.2*/, 1.5 );
  ...
  if ( selectionFlag == 7 ) latex->DrawLatex(0.60,0.20,Form("%.3f < y < %.3f",-4.000,  -2.500));
  if ( selectionFlag == 8 ) latex->DrawLatex(0.60,0.20,Form("%.3f < y < %.3f",-4.000,  -3.250));
  if ( selectionFlag == 9 ) latex->DrawLatex(0.60,0.20,Form("%.3f < y < %.3f",-3.250,  -2.500));
  if ( selectionFlag == 10) latex->DrawLatex(0.60,0.20,Form("%.3f < y < %.3f",-4.000,  -3.500));
  if ( selectionFlag == 11) latex->DrawLatex(0.60,0.20,Form("%.3f < y < %.3f",-3.500,  -3.000));
  if ( selectionFlag == 12) latex->DrawLatex(0.60,0.20,Form("%.3f < y < %.3f",-3.000,  -2.500));
  if ( selectionFlag == 13) latex->DrawLatex(0.60,0.20,Form("%.3f < y < %.3f",-4.000,  -3.625));
  if ( selectionFlag == 14) latex->DrawLatex(0.60,0.20,Form("%.3f < y < %.3f",-3.625,  -3.250));
  if ( selectionFlag == 15) latex->DrawLatex(0.60,0.20,Form("%.3f < y < %.3f",-3.250,  -2.875));
  if ( selectionFlag == 16) latex->DrawLatex(0.60,0.20,Form("%.3f < y < %.3f",-2.875,  -2.500));
  if ( selectionFlag == 17) latex->DrawLatex(0.60,0.20,Form("%.3f < y < %.3f",-4.000,  -3.700));
  if ( selectionFlag == 18) latex->DrawLatex(0.60,0.20,Form("%.3f < y < %.3f",-3.700,  -3.400));
  if ( selectionFlag == 19) latex->DrawLatex(0.60,0.20,Form("%.3f < y < %.3f",-3.400,  -3.100));
  if ( selectionFlag == 20) latex->DrawLatex(0.60,0.20,Form("%.3f < y < %.3f",-3.100,  -2.800));
  if ( selectionFlag == 21) latex->DrawLatex(0.60,0.20,Form("%.3f < y < %.3f",-2.800,  -2.500));
  ...
  cout << TotalPreliminaryIntegral << endl;
  cout << HighPtPreliminaryIntegral << endl;
