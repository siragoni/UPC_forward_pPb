...
void fitPtDistr(const char* AnalysisName, const int selectionFlag){
  ...
  fileMC[3] = new TFile("AnalysisResultsIncohMC_LHC16r_propertrigger_ADdecisions.root");
...
//  fModelForHighPtTail->SetParameter(2,debug==4 ? 6.1 : 1.);
  ...
  Double_t kFeedDownCoherent = 0.09;     // neutral element
  ...
  if        ( selectionFlag == 0 ) {
    FitPtDistr->SetParameter(1, 1000);
    FitPtDistr->SetParameter(2, 80);
  } else if ( selectionFlag == 1 ) {
    FitPtDistr->SetParameter(1, 1000);
    FitPtDistr->SetParameter(2, 80);
  } else if ( selectionFlag == 2 ) {
    FitPtDistr->SetParameter(1, 500);
    FitPtDistr->SetParameter(2, 40);
  } else if ( selectionFlag == 3 ) {
    FitPtDistr->SetParameter(1, 500);
    FitPtDistr->SetParameter(2, 40);
  } else if ( selectionFlag == 4 ) {
    FitPtDistr->SetParameter(1, 800);
    FitPtDistr->SetParameter(2, 66);
  } else if ( selectionFlag == 5 ) {
    FitPtDistr->SetParameter(1, 400);
    FitPtDistr->SetParameter(2, 33);
  ...
  FitPtDistr->SetParLimits(0, 0.0, 200.0);
  FitPtDistr->SetParLimits(1, FitPtDistr->GetParameter(1)*0.5, FitPtDistr->GetParameter(1)*2);
  ...
  Double_t errun = 0;
  Double_t errfI = 0;
  Double_t int1c = fCohJpsiToMuC  ->IntegralAndError( 1, fCohJpsiToMuC->GetXaxis()->FindBin(1.499999), err1c, "" );
  Double_t int1i = fIncohJpsiToMuC->IntegralAndError( 1, fCohJpsiToMuC->GetXaxis()->FindBin(1.499999), err1i, "" );
  Double_t intun = fHighPtTailC   ->IntegralAndError( 1, fCohJpsiToMuC->GetXaxis()->FindBin(1.499999), errun, "" );
  ...
  latex->DrawLatex(0.17,0.86,Form("f_{c} = #frac{%.3f + %.3f}{%.3f} = %.3f #pm %.7f", int1c, intun, int1i, f_I, errfI));
